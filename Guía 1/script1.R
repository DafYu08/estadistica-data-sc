...
#Esta forma de escribir todo generalizado es todavia mas comoda para poder usar ggplot
df <- data.frame(
...
#El metodo recontra funciona. Che esto ya lo hice, metemos el log jajajaja
...
#VEAMOS COMO REALIZAR UNA EMPIRICA Y COMO SE COMPORTA COMPARADO CON LA NORMAL

#Observemos cómo se ve la comparación con la normal antes de aplicar logaritmo...
...
#Y después de aplicar logaritmo
...
'Podemos sacar las mismas conclusiones que antes'


#Por último, para que después no me digan nada de que no tengo ninguna proba hecha,
#Supongamos que quiero saber la proba de que una nube tire menos de 40 mm sabiendo que es control
#Es decir P(x <=40 | x in CONTROLES ) 
